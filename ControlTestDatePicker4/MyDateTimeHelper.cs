...
{
    internal static class MyDateTimeHelper
    {
        ...
        {
            return MyDateTimeHelper.AddYears(date, year - date.Year);
        }
        ...
        public static DateTime? SetYearMonth(DateTime date, DateTime yearMonth)
        {
            ...
            if (target.HasValue)
            {
                target = MyDateTimeHelper.AddMonths(target.Value, yearMonth.Month - date.Month);
            }
